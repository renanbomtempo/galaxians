...
namespace moxxi
{
    ...
    Mesh ResourceManager::LoadMesh(std::string name, const char *path)   
    {
        Meshes[name] = LoadMeshFromOBJ(path);
        return Meshes[name];
    } 

    Mesh ResourceManager::LoadMeshFromOBJ(const char *path)
    {
        ...
        if (file == NULL)
        {
            std::cerr << "ERROR::MESH Failed to load OBJ file" << std::endl;
        }
        ...
        while (true)
        {
            ...
            else if (strcmp(lineHeader, "f") == 0)
            {
                ...
                if (matches != 3)
                {
                    printf("We only support triangles\n");
                }
        ...
        Mesh mesh;
        mesh.LoadMeshData(temp_vertices, temp_indices);
        return mesh;
    }

    Mesh ResourceManager::GetMesh(std::string name)
    {
        return ResourceManager::Meshes[name];
    }
